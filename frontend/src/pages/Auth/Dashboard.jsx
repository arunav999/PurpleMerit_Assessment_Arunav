...
export default function Dashboard() {
  ...
  const toggleStatus = async (userId, currentStatus) => {
    ...
    if (
      !window.confirm(`Are you sure you want to make this user ${newStatus}?`)
    )
      return;
    ...
    try {
      await axiosClient.patch(
        API_PATHS.USERS.UPDATE_USER_STATUS_BY_ID(userId),
        {
          status: newStatus,
        }
      );
      ...
      {currentUser?.role === "admin" ? (
        ...
        <div className="rounded-xl bg-white p-10 text-center shadow border border-gray-200">
          ...
          <p className="mt-2 text-gray-500">
            You have standard access. Contact an admin to upgrade.
          </p>
        </div>
      )}
    </div>
